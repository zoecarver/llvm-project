...
struct ConstSizeMemberDisabled {
...
static_assert(!std::is_invocable_v<RangeSizeT, SizeMemberDisabled>);
static_assert(!std::is_invocable_v<RangeSizeT, const ConstSizeMemberDisabled>);
static_assert(!std::is_invocable_v<RangeSizeT, SizeFunctionDisabled>);
static_assert(!std::is_invocable_v<RangeSizeT, const ConstSizeFunctionDisabled>);
...
constexpr int operator-(const sentinel, const other_forward_iterator) { return 2; }
constexpr int operator-(const other_forward_iterator, const sentinel) { return 2; }

static_assert(!std::same_as<other_forward_iterator::difference_type, int>);
...
constexpr long operator-(const sentinel, const random_access_iterator<int*>) { return 2; }
constexpr long operator-(const random_access_iterator<int*>, const sentinel) { return 2; }
...
constexpr bool testRanges() {
  ...
  RandomAccesslRange d;
  assert(std::ranges::size(d) == 2);
  ASSERT_SAME_TYPE(decltype(std::ranges::size(d)), unsigned long);
