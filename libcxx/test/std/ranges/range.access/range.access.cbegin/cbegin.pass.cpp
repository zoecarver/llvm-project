...
static_assert(std::semiregular<std::remove_cv_t<cbegin_t> >);
static_assert(std::invocable<cbegin_t&, int (&)[10]>);
static_assert(!std::invocable<cbegin_t&, int(&&)[10]>);
static_assert(!std::invocable<cbegin_t&, int (&)[]>);
static_assert(!std::invocable<cbegin_t&, int(&&)[]>);
...
concept invocable_member_with_desired_result =
  std::invocable<cbegin_t&, T> &&
  requires(T&& t) {
    { std::ranges::cbegin(std::forward<T>(t)) } -> std::same_as<std::decay_t<decltype(std::as_const(t).begin())> >;
  };
...
[[nodiscard]] consteval bool check_invocable_lvalue_member() {
  constexpr bool result = invocable_member_with_desired_result<T<enable_constness::const_only, is_borrowed>&>;
  static_assert(!invocable_member_with_desired_result<T<enable_constness::mutable_only, is_borrowed>&>);
...
[[nodiscard]] consteval bool check_invocable_rvalue_member() {
  constexpr bool result = invocable_member_with_desired_result<T<enable_constness::const_only, is_borrowed> const&&>;
  static_assert(!invocable_member_with_desired_result<T<enable_constness::mutable_only, is_borrowed>&&>);
  static_assert(!invocable_member_with_desired_result<T<enable_constness::mutable_only, is_borrowed> const&&>);
  static_assert(!invocable_member_with_desired_result<T<enable_constness::const_only, is_borrowed>&&>);
  static_assert(!invocable_member_with_desired_result<T<enable_constness::both, is_borrowed>&&>);
  static_assert(invocable_member_with_desired_result<T<enable_constness::both, is_borrowed> const&&> == result);
  return result;
}
...
[[nodiscard]] consteval bool check_invocable_lvalue_adl() {
  constexpr bool result = invocable_adl_with_desired_result<T<enable_constness::const_only, is_borrowed> const&>;
  static_assert(!invocable_adl_with_desired_result<T<enable_constness::mutable_only, is_borrowed>&>);
  static_assert(!invocable_adl_with_desired_result<T<enable_constness::mutable_only, is_borrowed> const&>);
  static_assert(invocable_adl_with_desired_result<T<enable_constness::const_only, is_borrowed>&> == result);
...
[[nodiscard]] consteval bool check_invocable_rvalue_adl() {
  ...
  static_assert(!invocable_adl_with_desired_result<T<enable_constness::both, is_borrowed>&&>);
  static_assert(invocable_adl_with_desired_result<T<enable_constness::both, is_borrowed> const&&> == result);
  return result;
}
...
static_assert(!std::invocable<cbegin_t&, empty&>);
static_assert(!std::invocable<cbegin_t&, empty const&>);
static_assert(!std::invocable<cbegin_t&, empty&&>);
static_assert(!std::invocable<cbegin_t&, empty const&&>);

static_assert(!std::invocable<cbegin_t&, bad_member_return&>);
static_assert(!std::invocable<cbegin_t&, bad_member_return const&>);
static_assert(!std::invocable<cbegin_t&, bad_member_return&&>);
static_assert(!std::invocable<cbegin_t&, bad_member_return const&&>);

static_assert(std::invocable<cbegin_t&, member_returns_decayable&>);
static_assert(std::invocable<cbegin_t&, member_returns_decayable const&>);

static_assert(std::invocable<cbegin_t&, adl_returns_decayable&>);
static_assert(std::invocable<cbegin_t&, adl_returns_decayable const&>);

ASSERT_NOEXCEPT(std::ranges::cbegin(std::declval<nothrow_member_begin<int*>&>()));
ASSERT_NOT_NOEXCEPT(std::ranges::cbegin(std::declval<nothrow_member_begin<ThrowingIterator<int> >&>()));
ASSERT_NOEXCEPT(std::ranges::cbegin(std::declval<nothrow_adl_begin<int*>&>()));
ASSERT_NOT_NOEXCEPT(std::ranges::cbegin(std::declval<nothrow_adl_begin<ThrowingIterator<int> >&>()));

template <typename T, std::same_as<T>... Ts>
constexpr bool check_lvalue_array(T t, Ts... ts) {
  T x[] = {t, ts...};
  return std::ranges::cbegin(x) == &x[0];
}

template <class T>
[[nodiscard]] constexpr bool check_member_impl() {
  std::remove_cvref_t<T> t;
  return std::ranges::cbegin(std::forward<T>(t)) == std::forward<T>(t).begin() && t.callee == called::member;
}
...
[[nodiscard]] constexpr bool check_adl_impl() {
  std::remove_cvref_t<T> t;
  using std::begin;
  return std::ranges::cbegin(std::forward<T>(t)) == begin(std::forward<T>(t)) && t.callee == called::adl;
}
