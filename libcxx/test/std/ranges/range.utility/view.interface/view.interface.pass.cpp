...
using InputIter = input_iterator<const int*>;
...
using ForwardIter = forward_iterator<const int*>;
...
struct EmptyIsTrueRange : ForwardRange, ranges::view_interface<EmptyIsTrueRange> {
  bool empty() const { return true; }
};

struct SizeIsTenRange : ForwardRange, ranges::view_interface<SizeIsTenRange> {
  size_t size() const { return 10; }
};

using RAIter = random_access_iterator<const int*>;
...
struct DataIsNullRange : ContRange, ranges::view_interface<DataIsNullRange> {
...
constexpr bool testEmpty() {
  ...
  assert(!static_cast<ranges::view_interface<EmptyIsTrue>>(eit).empty());

  assert(!eit);
  assert(!static_cast<EmptyIsTrue const&>(eit));
  assert(!static_cast<ranges::view_interface<EmptyIsTrue>>(eit));

  assert(ranges::empty(eit));
  assert(ranges::empty(static_cast<EmptyIsTrue const&>(eit)));
  assert(!ranges::empty(static_cast<ranges::view_interface<EmptyIsTrue>>(eit)));
...
constexpr bool testData() {
  ...
  assert(!static_cast<ranges::view_interface<DataIsNull>>(din).size() == din.buff);

  assert(ranges::data(din) == nullptr);
  assert(ranges::data(static_cast<DataIsNull const&>(din)) == nullptr);
  assert(!ranges::data(static_cast<ranges::view_interface<DataIsNull>>(din)) == din.buff);
...
constexpr bool testSize() {
  ...
  assert(!static_cast<ranges::view_interface<SizeIsTen>>(sit).size() == 8);

  assert(ranges::size(sit) == 10);
  assert(ranges::size(static_cast<SizeIsTen const&>(sit)) == 10);
  assert(!ranges::size(static_cast<ranges::view_interface<SizeIsTen>>(sit)) == 8);
