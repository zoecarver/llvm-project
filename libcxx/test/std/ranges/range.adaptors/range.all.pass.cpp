...
struct CopyableView : std::ranges::view_base {
  int start = 0;
  CopyableView() = default;
  constexpr explicit CopyableView(int start) : start(start) {}
...
struct Range {
  int start = 0;
  constexpr explicit Range(int start) : start(start) {}
...
struct BorrowableRange {
  int start = 0;
  constexpr explicit BorrowableRange(int start) : start(start) {}
...
constexpr bool test() {
  {
    ...
    auto viewCopy = std::views::all(View(2));
    ASSERT_SAME_TYPE(decltype(viewCopy), View);
    assert(std::ranges::begin(viewCopy) == globalBuff + 2);
    assert(std::ranges::end(viewCopy) == globalBuff + 8);
  }

  {
    ASSERT_SAME_TYPE(decltype(std::views::all(std::declval<const CopyableView&>())), CopyableView);

    CopyableView view(2);
    auto viewCopy = std::views::all(view);
    ASSERT_SAME_TYPE(decltype(viewCopy), CopyableView);
    assert(std::ranges::begin(viewCopy) == globalBuff + 2);
    assert(std::ranges::end(viewCopy) == globalBuff + 8);
  }
